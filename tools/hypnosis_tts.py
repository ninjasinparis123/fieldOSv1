...
OUT = Path('hypnosis_build')
OUT.mkdir(exist_ok=True)

# Short sections improve stability and let the model reset prosody naturally.
SECTIONS = [
...
def main():
    ...
    for i, text in enumerate(SECTIONS):
        ...
        if sr is None:
            sr = model.sr
        # Natural pause: 1.35 s between conceptual sections.
        waves.append(wav.cpu())
        pause = torch.zeros((wav.shape[0], int(sr * 1.35)))
        waves.append(pause)

    dry = torch.cat(waves, dim=-1)
    dry_path = OUT / 'voice_dry.wav'
    ta.save(str(dry_path), dry, sr)

    # Slightly slower delivery while preserving intelligibility.
    slow_path = OUT / 'voice_slow.wav'
    run(['ffmpeg','-y','-i',str(dry_path),'-filter:a','atempo=0.92',str(slow_path)])

    # Human voice stays centered; reverb and ambient layers are stereo and subtle.
    mix_path = OUT / 'hypnose_dunkelheit_licht.wav'
    filter_complex = (
        '[0:a]highpass=f=65,lowpass=f=14500,acompressor=threshold=-20dB:ratio=2.2:attack=25:release=180,'
        'asplit=2[dry][wet];'
        '[wet]aecho=0.7:0.35:80|145|230:0.18|0.12|0.07,volume=0.24[rv];'
        '[dry]volume=1.0[d];'
        '[1:a]volume=0.018[bn];'
        '[2:a]volume=0.010[left];[3:a]volume=0.010[right];'
        '[left][right]amerge=inputs=2[bi];'
        '[d][rv][bn][bi]amix=inputs=4:normalize=0,'
        'loudnorm=I=-18.0:LRA=5.0:TP=-1.0[out]'
    )
    run([
        'ffmpeg','-y','-i',str(slow_path),
        '-f','lavfi','-i','anoisesrc=color=brown:amplitude=0.025:r=48000',
        '-f','lavfi','-i','sine=frequency=174:sample_rate=48000',
        '-f','lavfi','-i','sine=frequency=180:sample_rate=48000',
        '-filter_complex',filter_complex,'-map','[out]','-shortest','-ar','48000','-ac','2',str(mix_path)
    ])

    mp3_path = OUT / 'Hypnose_Dunkelheit_zu_Licht_OpenSource.mp3'
    run(['ffmpeg','-y','-i',str(mix_path),'-codec:a','libmp3lame','-b:a','256k',str(mp3_path)])
    print('created:', mp3_path)
